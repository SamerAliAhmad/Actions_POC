...
namespace BLC
{
    ...
    public partial class BLC : IDisposable
    {
        ...
        {
            #region Body Section.
            OnPostEvent_Get_Dimension_By_OWNER_ID += new PostEvent_Handler_Get_Dimension_By_OWNER_ID(BLC_OnPostEvent_Get_Dimension_By_OWNER_ID);
            OnPostEvent_Edit_Dimension += new PostEvent_Handler_Edit_Dimension(BLC_OnPreEvent_Edit_Dimension);
            OnPostEvent_Edit_Dimension += new PostEvent_Handler_Edit_Dimension(BLC_OnPostEvent_Edit_Dimension);
            #endregion
        }
        #endregion
        ...
        #region BLC_OnPostEvent_Get_Dimension_By_OWNER_ID
        public void BLC_OnPostEvent_Get_Dimension_By_OWNER_ID(List<Dimension> i_Result, Params_Get_Dimension_By_OWNER_ID i_Params_Get_Dimension_By_OWNER_ID)
        {
            if (i_Result != null && i_Result.Count > 0)
            {
                foreach (Dimension oDimension in i_Result)
                {
                    if (oDimension.ICON_URL != null && !oDimension.ICON_URL.StartsWith(Global.Assets_Endpoint))
                    {
                        oDimension.ICON_URL = $"{Global.Assets_Endpoint}/{oDimension.ICON_URL}";
                    }
                }
            }
        }
        #endregion
        #region BLC_OnPreEvent_Edit_Dimension
        public void BLC_OnPreEvent_Edit_Dimension(Dimension i_Dimension, Enum_Edit_Mode i_Enum_Edit_Mode)
        {
            if (i_Dimension.ICON_URL != null && i_Dimension.ICON_URL.StartsWith(Global.Assets_Endpoint))
            {
                i_Dimension.ICON_URL = i_Dimension.ICON_URL.Remove(0, Global.Assets_Endpoint.Length + 1);
            }
        }
        #endregion
        #region BLC_OnPostEvent_Edit_Dimension
        public void BLC_OnPostEvent_Edit_Dimension(Dimension i_Dimension, Enum_Edit_Mode i_Enum_Edit_Mode)
        {
            if (i_Dimension.ICON_URL != null && !i_Dimension.ICON_URL.StartsWith(Global.Assets_Endpoint))
            {
                i_Dimension.ICON_URL = $"{Global.Assets_Endpoint}/{i_Dimension.ICON_URL}";
            }
        }
        #endregion
        #endregion
