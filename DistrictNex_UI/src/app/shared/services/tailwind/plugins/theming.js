...
const generateVariableColors = (theme) => {
    ...
    const customPropertiesWithOpacity = (name) => ({
        ...
        if (opacityValue) {
            return `rgba(var(--smartr-${name}-rgb), ${opacityValue})`;
        }
        if (opacityVariable) {
            return `rgba(var(--smartr-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--smartr-${name}-rgb))`;
    };
...
// -----------------------------------------------------------------------------------------------------
// @ SMARTR TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
const theming = plugin.withOptions((options) => ({
    ...
    const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
        ...
        ))), (value, key) => [[`--smartr-${e(key)}`, value], [`--smartr-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
    ]));
    ...
    const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) => {
        ...
        const background = theme(`smartr.customProps.background.${colorScheme}`);
        const foreground = theme(`smartr.customProps.foreground.${colorScheme}`);
        ...
        return {
            [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                ...
                ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--smartr-${e(key)}`, value], [`--smartr-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--smartr-${e(key)}`, value], [`--smartr-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            }
        };
    });
